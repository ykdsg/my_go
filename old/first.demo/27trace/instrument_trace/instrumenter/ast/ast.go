...
func (instrumenter instrumenter) Instrument(filename string) ([]byte, error) {
	...
	// 在AST上添加包导入语句
	astutil.AddImport(fset, curAST, instrumenter.traceImport)

	instrumenter.addDeferTraceIntoFuncDecls(curAST)
...
func (a instrumenter) addDeferStmt(fd *ast.FuncDecl) (added bool) {
	...
	newList[0] = ds // 注入新构造的defer语句，放在最前面
